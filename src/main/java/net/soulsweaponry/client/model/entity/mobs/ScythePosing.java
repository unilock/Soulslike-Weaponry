...
public abstract class ScythePosing {
    ...
    public static <T extends LivingEntity> void meleeAttack(ModelPart leftArm, ModelPart rightArm, T actor, float swingProgress, float oldAnimationProgress) {
        ...
        if (actor.getMainArm() == Arm.RIGHT) {
            leftArm.pitch = -0.5f + MathHelper.cos(oldAnimationProgress * 0.09f) * 0.15f;
            rightArm.pitch = -1.5f + MathHelper.cos(oldAnimationProgress * 0.19f) * 0.5f;
            leftArm.pitch += f * 1 - g;
            rightArm.pitch += f * 1 - g;
        } else {
            leftArm.pitch = -0.0f + MathHelper.cos(oldAnimationProgress * 0.19f) * 0.5f;
            rightArm.pitch = -1.8849558f + MathHelper.cos(oldAnimationProgress * 0.09f) * 0.15f;
            leftArm.pitch += f * 1.2f - g * 0.4f;
            rightArm.pitch += f * 2.2f - g * 0.4f;
        }
        CrossbowPosing.swingArms(leftArm, rightArm, oldAnimationProgress);
    }
}
